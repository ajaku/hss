...
combined_describe = pd.concat([smokers.describe(), non_smokers.describe()], axis=1, keys=['Smokers', 'Non-Smokers'])
combined_describe.to_excel("combined_describe_smokers_nonsmokers.xlsx")

# d)
(t_educ, p_educ) = ttest_ind(smokers['educ'], non_smokers['educ'], equal_var=False)
...
(t_income, p_income) = ttest_ind(smokers['income'], non_smokers['income'], equal_var=False)
